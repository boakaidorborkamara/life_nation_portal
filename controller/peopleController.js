...
const people_create_post = (req, res) => {
  ...
  (async () => {
    ...
    const whatsapp_group_link = await PrayerSchedule.findOne({
      where: { id: new_signup_details["PrayerScheduleId"] },
    });
    if (whatsapp_group_link === null) {
      ...
      res.send({
        status_code: 0,
        status_message: "Signup Sucessful",
        link: whatsapp_group_link["whatsAppLink"],
        available_time: whatsapp_group_link["time"],
      });
      // res.redirect('/thanks')
    }
  })();
